...
class Metric:
    def __init__(self,xpath,prometheus_job_name,prometheus_job_description):
        self.metric_name = xpath.split('/')[-1]
...
class Telemetry:
    ...
    def collect_metrics(self):
        for subscribe_response in self.client.subscribe([self.subscription_list]):
            if subscribe_response.update != []:
              metric_recieved = subscribe_response.update.update[0].path.elem[0].name
              metric_recieved_object = self.metric_name_object_map[metric_recieved]
              metric_recieved_value = subscribe_response.update.update[0].val.uint_val
              print(f"Metric : {metric_recieved}, Value : {metric_recieved_value}")
              metric_recieved_object.prometheus_metric.set(metric_recieved_value)
...
metrics_desired = [
            ...
            Metric("Cisco-IOS-XR-nto-misc-oper:memory-summary/nodes/node/summary/free-physical-memory","Cisco_XR_uptime_free_physical_memory","Physical Memory stats data connected from cisco telemetry"),
]
